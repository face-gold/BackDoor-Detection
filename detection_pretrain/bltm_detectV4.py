...
class BLTMTrainer(defense):
    ...
    def stable_dmi_loss(self,noisy_probs, noisy_labels, num_classes=10, epsilon=1e-6):
        """
        noisy_probs: [Batch, C] - 从 T 中提取出的预测噪声分布
        noisy_labels: [Batch] - 实际观察到的噪声标签
        """
        device = noisy_probs.device
        
        # 1. 构建 One-hot 标签 [Batch, C]
        target_one_hot = F.one_hot(noisy_labels, num_classes).float()
        
        # 2. 计算联合分布矩阵 M [C, C]
        # M = (P^T * Y) / Batch_Size
        mat = torch.matmul(noisy_probs.t(), target_one_hot)
        mat = mat / noisy_probs.size(0)
        
        # 3. 核心改进：添加微小单位阵增强稳健性，防止矩阵奇异
        mat = mat + epsilon * torch.eye(num_classes).to(device)
        
        # 4. 使用 slogdet 计算对数行列式
        # sign 是符号（1 或 -1），logabsdet 是行列式绝对值的自然对数
        sign, logabsdet = torch.slogdet(mat)
        
        # 如果行列式性质极差（sign <= 0），则返回一个较大的惩罚值或 0 梯度
        if sign <= 0:
            return torch.tensor(27.0, device=device, requires_grad=True) 
    ...
    def train(self):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for batch_idx, (data, bayes_labels, noisy_labels) in pbar:
                data, bayes_labels, noisy_labels = data.to(self.device), bayes_labels.to(self.device), noisy_labels.to(self.device)
                
                # 1. 前向传播得到转移矩阵 [Batch, 10, 10]
                T_pred = T_model(data) 
                
                # 2. 提取对应 Bayes 标签的行
                bayes_one_hot = F.one_hot(bayes_labels, 10).float().unsqueeze(1)
                noisy_class_post = torch.bmm(bayes_one_hot, T_pred).squeeze(1)
                
                # 3. 计算稳定的 DMI Loss
                loss = self.stable_dmi_loss(noisy_class_post, noisy_labels, num_classes=self.args.num_classes)
                
                # 4. 优化
                optimizer.zero_grad()
                loss.backward()
                
                # 梯度裁剪：防止 DMI 计算过程中出现极端梯度导致模型崩坏
                torch.nn.utils.clip_grad_norm_(T_model.parameters(), max_norm=5.0)
