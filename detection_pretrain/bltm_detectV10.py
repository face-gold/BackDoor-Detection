...
class BLTMTrainer(defense):
    ...
    def _evaluate_matrix_evolution(self, T_model, train_loader, epoch, matrix_history):
        """
        评估当前epoch的所有样本转移矩阵非对角线列和（按类别分类）
        """
        T_model.eval()
        args = self.args
        ...
        with torch.no_grad():
            for data, target, original_index, poison_indicator, original_target in tqdm(train_loader, desc=f"  Epoch {epoch} Matrix Eval"):
                data = data.to(self.device)
                poison_indicator = poison_indicator.to(self.device)
                
                T_pred = T_model(data)
                all_T.append(T_pred.cpu().numpy())
                all_is_poison.append(poison_indicator.cpu().numpy())
        
        all_T = np.concatenate(all_T, axis=0)  # [N, num_classes, num_classes]
        all_is_poison = np.concatenate(all_is_poison, axis=0)  # [N]
        
        # 计算每个样本的非对角线列和（每一列对应一个类）
        all_off_diag_sums = np.zeros((all_T.shape[0], num_classes))
        for i in range(all_T.shape[0]):
            all_off_diag_sums[i] = np.sum(all_T[i] * mask_off_diag, axis=0)
        
        # 初始化字典（在第一个epoch）
        if not matrix_history['epochs']:
            matrix_history['by_class'] = {}
            for class_idx in range(num_classes):
                matrix_history['by_class'][class_idx] = {
                    'clean_means': [],
                    'poison_means': [],
                    'clean_stds': [],
                    'poison_stds': [],
        ...
        # 分别处理每个类
        clean_mask = all_is_poison == 0
        poison_mask = all_is_poison == 1
        
        print(f"  Epoch {epoch} - Per-Class Analysis:")
        print(f"  {'Class':<8} {'Clean Mean':<15} {'Poison Mean':<15} {'Diff':<12} {'Samples':<10}")
        print(f"  {'-'*60}")
        
        for class_idx in range(num_classes):
            clean_sums = all_off_diag_sums[clean_mask, class_idx]
            poison_sums = all_off_diag_sums[poison_mask, class_idx]
            
            clean_mean = clean_sums.mean() if len(clean_sums) > 0 else 0
            poison_mean = poison_sums.mean() if len(poison_sums) > 0 else 0
            clean_std = clean_sums.std() if len(clean_sums) > 0 else 0
            poison_std = poison_sums.std() if len(poison_sums) > 0 else 0
            
            # 存储历史
            matrix_history['by_class'][class_idx]['clean_means'].append(clean_mean)
            matrix_history['by_class'][class_idx]['poison_means'].append(poison_mean)
            matrix_history['by_class'][class_idx]['clean_stds'].append(clean_std)
            matrix_history['by_class'][class_idx]['poison_stds'].append(poison_std)
            matrix_history['by_class'][class_idx]['clean_counts'].append(len(clean_sums))
            matrix_history['by_class'][class_idx]['poison_counts'].append(len(poison_sums))
            
            diff = poison_mean - clean_mean
            total_samples = len(clean_sums) + len(poison_sums)
            print(f"  {class_idx:<8} {clean_mean:<15.4f} {poison_mean:<15.4f} {diff:<12.4f} {total_samples:<10}")
        
        matrix_history['epochs'].append(epoch)
        T_model.train()
    
    def _visualize_matrix_evolution(self, matrix_history):
        """
        可视化每个类的转移矩阵非对角线列和随epoch的变化
        """
        if not matrix_history['epochs']:
            print("No matrix history to visualize.")
            return
        
        epochs = np.array(matrix_history['epochs'])
        num_classes = len(matrix_history['by_class'])
        
        # 绘制每个类的演化曲线（2x5网格，共10个类）
        ...
        for class_idx in range(num_classes):
            ...
            clean_means = np.array(matrix_history['by_class'][class_idx]['clean_means'])
            poison_means = np.array(matrix_history['by_class'][class_idx]['poison_means'])
            clean_stds = np.array(matrix_history['by_class'][class_idx]['clean_stds'])
            poison_stds = np.array(matrix_history['by_class'][class_idx]['poison_stds'])
            
            # 绘制曲线
            ax.plot(epochs, clean_means, 'g-o', label='Clean', linewidth=2, markersize=5)
            ax.plot(epochs, poison_means, 'r-s', label='Poisoned', linewidth=2, markersize=5)
            ...
            ax.fill_between(epochs, 
                            poison_means - poison_stds, 
                            poison_means + poison_stds,
                            alpha=0.2, color='red')
            
            ax.set_xlabel('Epoch', fontsize=10)
            ax.set_ylabel('Off-Diag Sum', fontsize=10)
            ax.set_title(f'Class {class_idx}', fontsize=11, fontweight='bold')
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3)
        
        plt.suptitle('T-Net Training: Per-Class Off-Diagonal Sum Evolution', 
                    fontsize=14, fontweight='bold', y=0.995)
        plt.tight_layout()
        
        plot_path = os.path.join(self.args.save_path, 'matrix_evolution_per_class.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"   -> [Saved] Per-class matrix evolution to {plot_path}")
        
        # 保存详细数据
        npz_data = {'epochs': epochs}
        for class_idx in range(num_classes):
            npz_data[f'class_{class_idx}_clean_means'] = np.array(matrix_history['by_class'][class_idx]['clean_means'])
            npz_data[f'class_{class_idx}_poison_means'] = np.array(matrix_history['by_class'][class_idx]['poison_means'])
            npz_data[f'class_{class_idx}_clean_stds'] = np.array(matrix_history['by_class'][class_idx]['clean_stds'])
            npz_data[f'class_{class_idx}_poison_stds'] = np.array(matrix_history['by_class'][class_idx]['poison_stds'])
        
        npz_path = os.path.join(self.args.save_path, 'matrix_evolution_per_class.npz')
        np.savez(npz_path, **npz_data)
        
        print(f"   -> [Saved] Per-class matrix history to {npz_path}")
        
        # 额外：绘制目标类识别过程（可选）
        print("\n[Target Class Detection Analysis]")
        print(f"{'Epoch':<8} {'Target Class':<15} {'Target Mean':<15} {'2nd Place':<15}")
        print("-" * 53)
        
        for epoch_idx, epoch in enumerate(epochs):
            means_at_epoch = [matrix_history['by_class'][c]['poison_means'][epoch_idx] 
                            for c in range(num_classes)]
            target_class = np.argmax(means_at_epoch)
            target_mean = means_at_epoch[target_class]
            
            # 找第二高的
            sorted_indices = np.argsort(means_at_epoch)[::-1]
            second_class = sorted_indices[1]
            second_mean = means_at_epoch[second_class]
            
            print(f"{epoch:<8} {target_class:<15} {target_mean:<15.4f} {second_mean:<15.4f}")
    ...
    def train(self):
        ...
        # 在训练循环前添加
        checkpoint_interval = 5  # 每5个epoch保存一次checkpoint和评估
        matrix_history = {
            'epochs': [],
            'by_class': {}  
        }
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            # 周期性评估：追踪转移矩阵演化
            if epoch % checkpoint_interval == 0 or epoch == args.epochs:
                print(f"\n[Checkpoint {epoch}] Evaluating matrix evolution...")
                
                # 保存checkpoint
                checkpoint_path = os.path.join(self.args.save_path, f'T_model_epoch_{epoch}.pth')
                torch.save(T_model.state_dict(), checkpoint_path)
                
                # 追踪矩阵演化
                self._evaluate_matrix_evolution(
                    T_model, train_loader, epoch, matrix_history
                )
        ...
        # 保存和可视化矩阵演化
        self._visualize_matrix_evolution(matrix_history)
    ...
    def visualize_kmeans_clustering_v2_with_p1_union(self):
        ...
        # 提取U中每个样本的转移矩阵特征（所有列的非对角线元素和）
        u_matrix_features = []
        for idx in union_indices:
            sample_off_diag_sums = np.sum(all_T[idx] * mask_off_diag, axis=0)
            u_matrix_features.append(sample_off_diag_sums)  # 保留所有列，不只是目标类

        u_matrix_features = np.array(u_matrix_features)  # [U_size, num_classes]

        print(f"  Matrix Features Shape: {u_matrix_features.shape}") 
        ...
        combined_features = np.concatenate([
            u_features_norm,
            u_matrix_features_norm  # 现在是[U_size, 10]维
        ], axis=1)  # [U_size, 522]
        ...
        kmeans_matrix_only_labels = gmm_matrix_only.fit_predict(u_matrix_features_norm)
