...
class BLTMTrainer(defense):
    ...
    def visualize_kmeans_with_p1_s1_intersection(self):
        ...
        # ==== Step 3.5: 优化P1集合得到P2（基于P1平均转移矩阵） ====
        logger.info("\n[Step 2.5] Refining P1 to P2 - Based on Average T-matrix of P1...")
        
        if len(p1_indices) > 0:
            # 计算P1的平均转移矩阵
            p1_T = all_T[p1_indices]  # [P1_size, num_classes, num_classes]
            mp = np.mean(p1_T, axis=0)  # [num_classes, num_classes] - 平均矩阵
            
            # 从mp的目标类列提取非对角线和的平均值
            mp_off_diag_sums = np.sum(mp * mask_off_diag, axis=0)  # [num_classes]
            mp_threshold = mp_off_diag_sums[predicted_target_class]  # 标量
            
            logger.info(f"  P1 Average T-matrix (target column {predicted_target_class}) off-diag threshold: {mp_threshold:.6f}")
            
            # 对P1进行进一步筛选：保留目标类非对角线列和 >= 平均值的样本
            p2_indices = []
            
            for idx in p1_indices:
                sample_off_diag_sums = np.sum(all_T[idx] * mask_off_diag, axis=0)
                sample_target_col_sum = sample_off_diag_sums[predicted_target_class]
                
                if sample_target_col_sum >= mp_threshold:
                    p2_indices.append(idx)
            
            p2_indices = np.array(p2_indices)
            
            # P2统计信息
            logger.info(f"  P2 Size: {len(p2_indices)} / {num_samples} ({100*len(p2_indices)/num_samples:.2f}%)")
            logger.info(f"  P2 Clean: {np.sum(all_is_poison[p2_indices] == 0)}")
            logger.info(f"  P2 Poison: {np.sum(all_is_poison[p2_indices] == 1)}")
            
            if len(p2_indices) > 0:
                p2_poison_ratio = np.sum(all_is_poison[p2_indices] == 1) / len(p2_indices)
                p1_poison_ratio = np.sum(all_is_poison[p1_indices] == 1) / len(p1_indices)
                logger.info(f"  [Improvement] P2 Poison Ratio: {p2_poison_ratio:.2%} (vs P1: {p1_poison_ratio:.2%})")
            else:
                logger.info("  [WARNING] P2 is empty after filtering!")
                p2_indices = p1_indices  # 回退到P1
        else:
            logger.info("  [WARNING] P1 is empty, skipping P2 refinement")
            p2_indices = np.array([])
        ...
        def align_and_evaluate_with_matrix_feature(pred_labels, true_labels, u_matrix_features, 
            ...
            # 计算P2内所有样本的索引集合（用于快速查询）
            p2_set = set(p2_indices)
            
            # 对于每个簇，找出既在该簇中又在P2中的样本
            cluster_0_mask = pred_labels == 0
            cluster_1_mask = pred_labels == 1
            
            # 簇0中同时在P2中的样本
            cluster_0_in_p2_mask = np.array([
                (cluster_0_mask[i] and u_indices[i] in p2_set) 
                for i in range(len(u_indices))
            ])
            
            # 簇1中同时在P2中的样本
            cluster_1_in_p2_mask = np.array([
                (cluster_1_mask[i] and u_indices[i] in p2_set) 
                for i in range(len(u_indices))
            ])
            
            # 计算两个簇的平均矩阵特征（仅考虑P2中的样本）
            if np.sum(cluster_0_in_p2_mask) > 0:
                cluster_0_avg_matrix_feature = np.mean(u_matrix_features[cluster_0_in_p2_mask])
                cluster_0_p2_count = np.sum(cluster_0_in_p2_mask)
            else:
                cluster_0_avg_matrix_feature = 0
                cluster_0_p2_count = 0
            
            if np.sum(cluster_1_in_p2_mask) > 0:
                cluster_1_avg_matrix_feature = np.mean(u_matrix_features[cluster_1_in_p2_mask])
                cluster_1_p2_count = np.sum(cluster_1_in_p2_mask)
            else:
                cluster_1_avg_matrix_feature = 0
                cluster_1_p2_count = 0
            ...
            log(f"\n{method_name}:")
            log(f"  [P2 Filtering Applied]")
            log(f"  Cluster 0: {np.sum(cluster_0_mask)} samples, {cluster_0_p2_count} in P2, avg matrix feature: {cluster_0_avg_matrix_feature:.4f}")
            log(f"  Cluster 1: {np.sum(cluster_1_mask)} samples, {cluster_1_p2_count} in P2, avg matrix feature: {cluster_1_avg_matrix_feature:.4f}")
            log(f"  → Cluster {poison_cluster} identified as Poisoned (higher avg matrix feature in P2)")
