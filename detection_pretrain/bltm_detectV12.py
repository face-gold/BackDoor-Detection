...
class BLTMTrainer(defense):
    ...
    def visualize_kmeans_with_p1_s1_intersection(self):
        ...
        print("="*70)
        print("Enhanced K-Means: P1 ∩ S1 Intersection (True Target Label)")
        print("="*70)
        ...
        print(f"  Off-diagonal sum in target class: {global_off_diag_sums[predicted_target_class]:.4f}")
        ...
        p1_indices = np.array(p1_indices)
        print(f"  P1 Size: {len(p1_indices)} / {num_samples} ({100*len(p1_indices)/num_samples:.2f}%)")
        print(f"  P1 Clean: {np.sum(all_is_poison[p1_indices] == 0)}")
        print(f"  P1 Poison: {np.sum(all_is_poison[p1_indices] == 1)}")
        ...
        print(f"  S1 Size: {len(s1_indices)} / {num_samples} ({100*len(s1_indices)/num_samples:.2f}%)")
        print(f"  S1 Clean: {np.sum(all_is_poison[s1_indices] == 0)}")
        print(f"  S1 Poison: {np.sum(all_is_poison[s1_indices] == 1)}")
        ...
        print(f"  Intersection Size: {len(intersection_indices)} samples")
        
        if len(intersection_indices) == 0:
            print("\n[WARNING] Intersection is empty!")
            print("  Falling back to Union (P1 ∪ S1)...")
            union_indices = np.union1d(p1_indices, s1_indices)
            u_indices = union_indices
            use_intersection = False
            print(f"  Union Size: {len(u_indices)} samples")
        else:
        ...
        print(f"  U Clean Samples: {np.sum(u_labels == 0)}")
        print(f"  U Poison Samples: {np.sum(u_labels == 1)}")
        print(f"  U Poison Ratio: {np.sum(u_labels == 1) / len(u_labels):.2%}")
        
        # ==== Step 7: 提取转移矩阵特征 ====
        print(f"\n[Step 6] Computing Transition Matrix Features (Target Class Only)...")
        u_matrix_features = []
        ...
        u_matrix_features = np.array(u_matrix_features).reshape(-1, 1)  # [U_size, 1]
        print(f"  Matrix Features Shape: {u_matrix_features.shape}")
        ...
        # 方法1: 标准特征空间聚类
        print("  Method 1: Standard Feature Space (512-dim)...")
        gmm_std = GaussianMixture(n_components=2, random_state=42, n_init=10, covariance_type='full')
        labels_std = gmm_std.fit_predict(u_features)

        # 方法2: 结合转移矩阵特征
        print("  Method 2: Dual-Feature Space (512 + 1 dims)...")
        u_features_norm = (u_features - u_features.mean(axis=0)) / (u_features.std(axis=0) + 1e-10)
        u_matrix_features_norm = (u_matrix_features - u_matrix_features.mean()) / (u_matrix_features.std() + 1e-10)

        combined_features = np.concatenate([u_features_norm, u_matrix_features_norm], axis=1)  # [U_size, 513]
        labels_dual = KMeans(n_clusters=2, random_state=42, n_init=10).fit_predict(combined_features)

        # 方法3: 仅转移矩阵特征
        print("  Method 3: Matrix Feature Only (1-dim)...")
        labels_matrix = GaussianMixture(n_components=2, random_state=42, n_init=10).fit_predict(u_matrix_features)
        ...
        def align_and_evaluate_with_matrix_feature(pred_labels, true_labels, u_matrix_features, method_name):
            ...
            # Step 1: 计算两个簇的平均矩阵特征
            cluster_0_avg_matrix_feature = u_matrix_features[pred_labels == 0].mean() if np.sum(pred_labels == 0) > 0 else 0
            cluster_1_avg_matrix_feature = u_matrix_features[pred_labels == 1].mean() if np.sum(pred_labels == 1) > 0 else 0
        ...
        print("="*70)
        print(f"{'Method':<40} {'Accuracy':<12} {'Precision':<12} {'Recall':<12} {'F1-Score':<12}")
        print("-" * 88)
        print(f"{'Std Features':<40} {metrics_std[0]:<12.4f} {metrics_std[1]:<12.4f} {metrics_std[2]:<12.4f} {metrics_std[3]:<12.4f}")
        print(f"{'Dual Features':<40} {metrics_dual[0]:<12.4f} {metrics_dual[1]:<12.4f} {metrics_dual[2]:<12.4f} {metrics_dual[3]:<12.4f}")
        print(f"{'Matrix Features Only':<40} {metrics_matrix[0]:<12.4f} {metrics_matrix[1]:<12.4f} {metrics_matrix[2]:<12.4f} {metrics_matrix[3]:<12.4f}")
        print("="*88)
        
        # 计算改进
        print("\n[IMPROVEMENT ANALYSIS]")
        print("-" * 88)
        print(f"Dual vs Std:     Recall +{(metrics_dual[2] - metrics_std[2])*100:.2f}pp, F1 +{(metrics_dual[3] - metrics_std[3])*100:.2f}pp")
        print(f"Matrix vs Std:   Recall +{(metrics_matrix[2] - metrics_std[2])*100:.2f}pp, F1 +{(metrics_matrix[3] - metrics_std[3])*100:.2f}pp")
        print("="*88 + "\n")
