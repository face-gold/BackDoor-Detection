...
class BLTMTrainer(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--class_subset_n', type=int,help='use first N classes (0..N-1); None means full dataset')
    ...
    def _get_subset_indices_by_original_target(self, bd_dataset, n_classes, train_tran):
        if n_classes is None:
            return None

        # 用 loader 拿到 original_target + original_index
        tmp_dataset = dataset_wrapper_with_transform(
            bd_dataset, wrap_img_transform=train_tran, wrap_label_transform=None
        )
        ...
        selected = []
        with torch.no_grad():
            for data, target, original_index, poison_indicator, original_target in tmp_loader:
                # original_target 是真实标签
                original_target = original_target.numpy()
                original_index = original_index.numpy()
                mask = original_target < n_classes
                selected.extend(original_index[mask].tolist())

        return np.array(selected)

    def _remap_targets_in_dataset(self, bd_dataset, n_classes):
        # 把 target / original_target 映射到 0..N-1
        # 只对 0..N-1 之间的标签做映射，保持范围一致
        if hasattr(bd_dataset, "targets"):
            bd_dataset.targets = [t if t < n_classes else t for t in bd_dataset.targets]
        if hasattr(bd_dataset, "original_targets"):
            bd_dataset.original_targets = [t if t < n_classes else t for t in bd_dataset.original_targets]
        if hasattr(bd_dataset, "original_target"):
            bd_dataset.original_target = [t if t < n_classes else t for t in bd_dataset.original_target]
    ...
    def train(self):
        ...
        #子集选择
        selected_indices = self._get_subset_indices_by_original_target(bd_dataset, self.args.class_subset_n, train_tran)
        if selected_indices is not None:
            bd_dataset = Subset(bd_dataset, selected_indices)
            # 严格 n 类实验：覆盖 num_classes
            self.args.num_classes = self.args.class_subset_n
            print(f"Selected first {self.args.num_classes} classes for training.")
    ...
    def save_all_matrices(self):
        ...
        # 子集选择（基于 original_target）
        selected_indices = self._get_subset_indices_by_original_target(
            bd_dataset, self.args.class_subset_n, train_tran
        )
        if selected_indices is not None:
            bd_dataset = Subset(bd_dataset, selected_indices)
            self.args.num_classes = self.args.class_subset_n
            print(f"Selected first {self.args.num_classes} classes for analysis.")

        # 现在再初始化模型，确保 num_classes 已更新
        t_net_output_dim = self.args.num_classes * self.args.num_classes
        T_model = preact_resnet_T.PreActResNet18(t_net_output_dim)
        ...
        C_model = preact_resnet.PreActResNet18(self.args.num_classes)
        ...
        with torch.no_grad():
            for data, target, original_index, poison_indicator_batch, original_target in tqdm(train_loader, desc="Inference"):
                ...
                all_is_poison.append(poison_indicator_batch.cpu().numpy())
        ...
        all_is_poison = np.concatenate(all_is_poison, axis=0)
        ...
        with open(txt_path, "w") as f:
            for idx, is_p, T, orig_label, bayes_lbl in zip(
                all_index, all_is_poison, all_T, all_original_targets, all_bayes_labels
            ):
                f.write(f"Index: {idx}\n")
    ...
    def visualize_kmeans_with_s1(self):
        """
        对S1进行无监督聚类（仅使用目标类真实标签样本）
        """
        logger = self.setup_logger(self.args.save_path)

        logger.info("="*60)
        logger.info("K-Means on S1 (Target Class Only)")
        logger.info("="*60)
        ...
        # ==== Step 3: 直接取S1集合（真实标签为目标类） ====
        logger.info("\n[Step 2] Filtering S1 - Based on training label...")
        ...
        u_indices = s1_indices  # 直接用S1
        use_intersection = True

        # ==== Step 4: 提取特征（与子集保持一致） ====
        logger.info(f"\n[Step 3] Extracting Features from {len(u_indices)} samples...")
        ...
        if all_features.shape[0] != all_T.shape[0]:
            logger.info("WARNING: feature count != all_T count, check subset alignment.")

        u_features = all_features[u_indices]
        u_labels = all_is_poison[u_indices]
        u_T = all_T[u_indices]
        ...
        # ==== Step 5: 提取转移矩阵特征 ====
        logger.info(f"\n[Step 4] Computing Transition Matrix Features...")
        u_matrix_features = []
        ...
        u_matrix_features = np.array(u_matrix_features)
        ...
        # ==== Step 7: t-SNE可视化 ====
        logger.info(f"\n[Step 6] Performing t-SNE Visualization...")
        ...
        plt.suptitle(
            f'Clustering on S1: Target Class {predicted_target_class}, Size: {len(u_indices)}',
            fontsize=14, fontweight='bold', y=0.995
        )
        plt.tight_layout()

        plot_path = os.path.join(self.args.save_path, 'kmeans_s1.png')
        ...
        def align_and_evaluate_with_matrix_feature(pred_labels, true_labels,
                                                u_matrix_features, method_name, logger=None):
            """
            pred_labels: 聚类标签 [U_size]
            true_labels: 真实毒性标签 [U_size] (0=clean, 1=poison)
            u_matrix_features: 矩阵特征 [U_size]，例如目标列非对角线和
            method_name: 方法名称
            logger: 日志记录器
            """
            ...
            if np.sum(cluster_0_mask) > 0:
                cluster_0_avg_matrix_feature = np.mean(u_matrix_features[cluster_0_mask])
                cluster_0_count = np.sum(cluster_0_mask)
            else:
                cluster_0_avg_matrix_feature = 0
                cluster_0_count = 0

            if np.sum(cluster_1_mask) > 0:
                cluster_1_avg_matrix_feature = np.mean(u_matrix_features[cluster_1_mask])
                cluster_1_count = np.sum(cluster_1_mask)
            else:
            ...
            log(f"\n{method_name}:")
            log("  [U-only Cluster Alignment]")
            log(f"  Cluster 0: {cluster_0_count} samples, avg matrix feature: {cluster_0_avg_matrix_feature:.4f}")
            log(f"  Cluster 1: {cluster_1_count} samples, avg matrix feature: {cluster_1_avg_matrix_feature:.4f}")
            log(f"  -> Cluster {poison_cluster} identified as Poisoned")
...
if __name__ == '__main__':
    ...
    trainer.visualize_kmeans_with_s1()