...
class BLTMTrainer(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--asr_threshold', type=float, help='ASR threshold for early stopping during warmup')
    ...
    def warmup_classifier(self, train_loader, clean_test_loader, bd_test_loader):
        """
        Phase 1: Warm-up Classifier with dynamic stop based on ASR.
        """
        print(f"===> Initializing PreActResNet18 and warming up (ASR Threshold: {self.args.asr_threshold:.2f})...")
        ...
        for epoch in range(1, self.args.warmup_epochs + 1):
            ...
            # --- 核心修改：动态评估 ASR ---
            acc, asr = self.evaluate_accuracy_and_asr(model, clean_test_loader, bd_test_loader)
            logging.info(f'Epoch {epoch}: Loss={total_loss/len(train_loader):.4f}, Clean Acc={acc:.2%}, ASR={asr:.2%}')
            print(f"\n[Monitor] Clean Acc: {acc:.2%}, ASR: {asr:.2%}")

            # 动态停止判断
            if asr >= asr_threshold and epoch > 5:
                print(f"!!! Target ASR {asr_threshold:.2%} reached ({asr:.2%}). Stopping Warm-up early.")
                break
            
            # 即使没达到阈值，也可以保存当前最好的状态
            if asr > best_asr:
                best_asr = asr
                torch.save(model.state_dict(), os.path.join(self.args.save_path, 'warmup_classifier_best_asr.pth'))
    ...
    def evaluate_accuracy_and_asr(self, model, clean_test_loader, bd_test_loader):
        """
        辅助函数：计算干净准确率 (ACC) 和攻击成功率 (ASR)
        """
        model.eval()
        # 1. 计算干净 ACC
        correct = 0
        total = 0
        with torch.no_grad():
            for data, target, *_ in clean_test_loader:
                data, target = data.to(self.device), target.to(self.device)
                outputs = model(data)
        ...
        total = 0
        with torch.no_grad():
            for data, target, *_ in bd_test_loader:
                data, target = data.to(self.device), target.to(self.device)
                outputs = model(data)
                _, predicted = outputs.max(1)
                total += target.size(0)
                # ASR 定义：预测为攻击者目标标签（target 在这里通常已是 target_label）
                correct += predicted.eq(target).sum().item()
        asr = correct / total
    ...
    def train(self):
        ...
        # 干净测试集
        clean_test_dataset = dataset_wrapper_with_transform(self.result['clean_test'].wrapped_dataset, wrap_img_transform=test_tran)
        clean_test_loader = DataLoader(clean_test_dataset, batch_size=self.args.batch_size, shuffle=False)
        
        # 中毒测试集
        bd_test_dataset = dataset_wrapper_with_transform(self.result['bd_test'].wrapped_dataset, wrap_img_transform=test_tran)
        bd_test_loader = DataLoader(bd_test_dataset, batch_size=self.args.batch_size, shuffle=False)

        # 2. Warm-up Classifier
        #classifier = self.warmup_classifier(train_loader)
        classifier = self.warmup_classifier(train_loader, clean_test_loader, bd_test_loader)
