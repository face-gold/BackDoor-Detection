    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for batch_data in self.train_loader:
                ...
                # 2. Volume Loss (Type 'w'使用混淆矩阵的体积正则——对标注者行为进行约束)——效果略差
                # P0_softmax = self.coin_model.P0_normalize(self.coin_model.P0)
                # W = P0_softmax.reshape(-1, self.num_classes)
                # WW = torch.mm(W.t(), W)
                # identity = torch.eye(self.num_classes).to(self.device)
                # logdet = torch.log(torch.linalg.det(WW + 1e-3 * identity))
